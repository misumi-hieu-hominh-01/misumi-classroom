...
export class MultiplayerGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	async handleConnection(client: Socket<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>) {
	...
	async handleDisconnect(client: Socket<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>) {
	...
	async handleJoinRoom(
		...
		@MessageBody() data: { roomId: string; userData: { userId: string; username: string } }
	) {
		try {
			// Validate data
			...
			if (client.data.roomId && client.data.playerId) {
				...
				this.multiplayerService.removePlayerFromRoom(client.data.roomId, client.data.playerId)
			}
	...
	async handleLeaveRoom(
		...
		try {
			...
			const removed = this.multiplayerService.removePlayerFromRoom(roomId, playerId)
	...
	async handlePlayerMove(
		@ConnectedSocket() client: Socket<ClientToServerEvents, ServerToClientEvents, InterServerEvents, SocketData>,
		@MessageBody() data: { position: PlayerPosition; rotation: PlayerRotation; isMoving: boolean }
	) {
		try {
			if (!client.data.roomId || !client.data.playerId) {
				this.logger.warn(`Player move received but player not in room: ${client.id}`)
				return
			}
			...
			if (
				...
				this.logger.error('Invalid position/rotation types:', { position, rotation, isMoving })
				return
			}
			...
			if (updatedPlayer) {
				...
				client.to(roomId).emit('player-moved', playerId, position, rotation, isMoving)
			} else {
				this.logger.warn(`Failed to update player position: ${playerId} in room ${roomId}`)
			}
		} catch (error) {
	...
	broadcastToRoom(roomId: string, event: keyof ServerToClientEvents, ...args: any[]) {
